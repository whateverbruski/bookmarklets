...
(function () {
  (async function () {
    ...
    try {
      ...
      alert("Couldn't read clipboard — copy your list first or grant permissions.");
      return;
    }
    ...
    if (decisions.length === 0 && groupsOrdered.length === 1 && groupsOrdered[0].heading === "") {
      alert("No items found in clipboard (expected lines starting with '.' for items).");
      return;
    }
    ...
    // 5) assemble + write — preserve lonely headings
    function assembleAndWrite() {
      let keptOut = "";
      let droppedOut = "";

      for (const g of groupsOrdered) {
        const kept = decisions.filter((d) => d.heading === g.heading && d.keep).map((d) => "." + d.text);
        const dropped = decisions.filter((d) => d.heading === g.heading && !d.keep).map((d) => "." + d.text);
      ...
      keptOut = keptOut.replace(/\s+$/g, "");
      droppedOut = droppedOut.replace(/\s+$/g, "");
    ...
    function render() {
      ...
      if (s.idx >= decisions.length) {
        ...
        assembleAndWrite()
          .then(() => { cleanup(); alert("Done — output written to clipboard."); })
          .catch(() => { cleanup(); alert("Couldn't write to clipboard."); });
        return;
      }
